...
void ModuleConsolePlugin::Install(const server::ConfigFile* config_file) {
  Plugin::Install(config_file);
}

void ModuleConsolePlugin::Initialize() {
  const char* listen_instance_name =
    config_file_->GetString(kConsoleModuleName.c_str(), "listen_instance_name");
  if (listen_instance_name) {
    listen_instance_name_ = listen_instance_name;
    listen_context_ = ZEngineContextManager::GetInstance()->
      LookupContext(listen_instance_name_);
  }
