...
bool ZXmlWrapper::ElementEnd(const std::string& element) {
  ...
  int result = script_->CallFunction<int, lua_tinker::table, const char*>(
    "ElementEnd", *table_, element.c_str());
  if (result!=0) {
    LOG(ERROR)
      << "ERROR: In main.lua, Execute ElementEnd() error, error_code = "
      << result;
  }
...
bool ZXmlWrapper::Text(const std::string& text) {
  ...
  int result = script_->CallFunction<int, lua_tinker::table, const char*>(
    "Text", *table_, text.c_str());
  if (result!=0) {
    LOG(ERROR)
      << "ERROR: In main.lua, Execute Text() error, error_code = "
      << result;
  }
