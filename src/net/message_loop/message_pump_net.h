...
#ifndef NET_MESSAGE_LOOP_MESSAGE_PUMP_NET_H_
#define NET_MESSAGE_LOOP_MESSAGE_PUMP_NET_H_
...
class MessagePumpNet : public base::MessagePump {
 public:
  class IOObserver {
   public:
    IOObserver() {}
  ...
  class FileDescriptorWatcher {
   ...
   private:
    friend class MessagePumpNet;
    friend class MessagePumpNetTest;

    // Called by MessagePumpNet, ownership of |e| is transferred to this
    // object.
    void Init(event* e);

    // Used by MessagePumpNet to take ownership of event_.
    event* ReleaseEvent();

    void set_pump(MessagePumpNet* pump) { pump_ = pump; }
    MessagePumpNet* pump() const { return pump_; }

    void set_watcher(Watcher* watcher) { watcher_ = watcher; }

    void OnFileCanReadWithoutBlocking(int fd, MessagePumpNet* pump);
    void OnFileCanWriteWithoutBlocking(int fd, MessagePumpNet* pump);

    event* event_;
    MessagePumpNet* pump_;
    Watcher* watcher_;
 ...
 private:
  ...
  base::ObserverList<IOObserver> io_observers_;
  base::ThreadChecker watch_file_descriptor_caller_checker_;
  DISALLOW_COPY_AND_ASSIGN(MessagePumpNet);
};

}  // namespace net

#endif // NET_MESSAGE_LOOP_MESSAGE_PUMP_NET_H_
