...
static int tostring_s64(lua_State *L) {
  ...
  std::string temp = base::Int64ToString(*reinterpret_cast<const int64_t*>(lua_topointer(L, 1)));
...
static int eq_s64(lua_State *L) {
  ...
  int64_t a = *reinterpret_cast<const int64_t*>(lua_touserdata(L, 1));
  int64_t b = *reinterpret_cast<const int64_t*>(lua_touserdata(L, 2));
...
static int tostring_u64(lua_State *L) {
  ...
  std::string temp = base::Uint64ToString(*reinterpret_cast<const uint64_t*>(lua_topointer(L, 1)));
  lua_pushstring(L, temp.c_str());
  return 1;
}
...
static int eq_u64(lua_State *L) {
  ...
  uint64_t a = *reinterpret_cast<const uint64_t*>(lua_touserdata(L, 1));
  uint64_t b = *reinterpret_cast<const uint64_t*>(lua_touserdata(L, 2));
  lua_pushboolean(L, (a == b));
  return 1;
}
...
static void call_stack(lua_State* L, int n) {
  ...
  if (lua_getstack(L, n, &ar) == 1) {
    ...
    if (ar.name) {
      lua_tinker::print_error(L, "%s%s() : line %d [%s : line %d]",
        indent, ar.name, ar.currentline, ar.source, ar.linedefined);
    } else {
      lua_tinker::print_error(L, "%sunknown : line %d [%s : line %d]",
