...
#include "server/base/config_file_util.h"
...
void DBAddrInfo::PrintDebugString() {
  std::cout << user << ":" << passwd << "@" << host << ":" << port
    << "/" << db_name << "/" << std::endl;
}

bool DBAddrInfo::ParseFromConfigFile(const ConfigFile& config_file,
...
bool IpAddrInfo::ParseFromConfigFile(const ConfigFile& config_file,
  const char* section_name) {
  if (!config_file.CheckBySection(section_name)) {
    LOG(ERROR) << "Not find section " << section_name;
    return false;
...
bool IpAddrInfo2::ParseFromConfigFile(const ConfigFile& config_file,
  const char* section_name) {
  if (!config_file.CheckBySection(section_name)) {
    LOG(ERROR) << "Not find section " << section_name;
    return false;
