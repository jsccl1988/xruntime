...
class SharedPtrControlBlock {
 ...
 private:
  ...
  base::subtle::Atomic32 refcount_;
  base::subtle::Atomic32 weak_count_;
};
...
class shared_ptr {
 ...
 public:
  ...
  ~shared_ptr() {
    if (ptr_ != NULL) {
      if (!base::AtomicRefCountDec(&control_block_->refcount_)) {
        delete ptr_;

        // weak_count_ is defined as the number of weak_ptrs that observe
        // ptr_, plus 1 if refcount_ is nonzero.
        if (!base::AtomicRefCountDec(&control_block_->weak_count_)) {
 ...
 private:
  ...
  void InitializeWithStaticCast(const shared_ptr<V>& r) {
    if (r.control_block_ != NULL) {
      base::AtomicRefCountInc(&r.control_block_->refcount_);
...
class weak_ptr {
 ...
 public:
  ...
  ~weak_ptr() {
    if (control_block_ != NULL &&
        !base::AtomicRefCountDec(&control_block_->weak_count_)) {
      delete control_block_;
    }
  }
  ...
  shared_ptr<T> lock() const {
    shared_ptr<T> result;
    if (control_block_ != NULL) {
      base::subtle::Atomic32 old_refcount;
      do {
 ...
 private:
  void CopyFrom(T* ptr, SharedPtrControlBlock* control_block) {
    ...
    if (control_block_ != NULL)
      base::AtomicRefCountInc(&control_block_->weak_count_);
  }
