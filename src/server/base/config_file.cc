...
const char* GetValueByKey(const server::ConfigFile::KeyValuePairList& key_values,
  const char* key, const char* default_val=NULL) {
...
void GetValueListByKey(const server::ConfigFile::KeyValuePairList& key_values,
  const char* key, std::vector<const char*>* values) {
  for (size_t i=0; i<key_values.size(); ++i) {
    if (key_values[i].first == key) {
...
bool ConfigFile::Load() {
  ...
  XMLLite xml_lite;
  bool result = xml_lite.GetXmlParser()->
    ParseXMLFile(xml_handler, config_file_path_.c_str());
  if (!result) {
    Destroy();
  }
  return result;
...
const char* ConfigFile::GetString(const char* section,
  const char* key, const char* default_val) const {
  const char* val = default_val;
...
StringVector ConfigFile::GetSectionName() const {
  StringVector keys;
