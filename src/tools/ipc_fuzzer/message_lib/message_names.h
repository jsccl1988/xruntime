...
class MessageNames {
 public:
  ...
  void Add(uint32_t_t type, const std::string& name) {
    name_map_[type] = name;
    type_map_[name] = type;
  }

  bool TypeExists(uint32_t_t type) {
  ...
  const std::string& TypeToName(uint32_t_t type) {
    TypeToNameMap::iterator it = name_map_.find(type);
    CHECK(it != name_map_.end());
    return it->second;
  }

  uint32_t_t NameToType(const std::string& name) {
 ...
 private:
  typedef std::map<uint32_t_t, std::string> TypeToNameMap;
  typedef std::map<std::string, uint32_t_t> NameToTypeMap;
